...
[![Built by LLM](https://img.shields.io/badge/Generated%20by-LLM%20%2F%20AI-8A2BE2.svg)](#)
...
3. **Hardware Handshake Settle:** Waits a configurable 150ms so the Windows audio driver and USB bus controller finish rate negotiation.
4. **Direct DLL Interop:** Dynamically loads `VoicemeeterRemote64.dll`, calls `VBVMR_Login()`, sets `Command.Restart = 1.0f`, and unloads.
...
Run the release binary with `install`:
```powershell
...
To specify a custom device name or settling delay:
```powershell
.\target\release\earplugger.exe install --device "RODE NT-USB" --delay-ms 150
```

### 3. Verify Status
```powershell
.\target\release\earplugger.exe status
```
